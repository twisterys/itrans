...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Magasinages  @endslot
         @slot('action')  <a class="btn btn-success " href="{{route('magasinage.create')}}">Nouveau magasinage </a>    @endslot
     @endcomponent
                                    ...
                                    <table id="datatable-buttons" class="table table-bordered data-table" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Date Entrée</th>
                                                <th>Date Sortie</th>
                                                <th>Matricule Entrée</th>
                                                <th>Matricule Sortie</th>
                                                <th>Depot</th>
                                                <th>Prix</th>
                                                <th>&nbsp;</th>
                                            </tr>
    ...
    <script type="text/javascript">
        $(function () {
          ...
          var table = $('#datatable-buttons').DataTable({
              processing: true,
              serverSide: true,
              ajax: "{{ route('magasinage.index') }}",
              columns: [
                  {data: 'id', name: 'id'},
                  {data: 'client', name: 'client'},
                  {data: 'date_entree', name: 'date_entree'},
                  {data: 'date_sortie', name: 'date_sortie'},
                  {data: 'mat_entree', name: 'mat_entree'},
                  {data: 'mat_sortie', name: 'mat_sortie'},
    ...
    table.column(index).search(value, strict).draw()
        ...
        function deleteMagasinage(id){
            ...
            if (e.value === true) {
                ...
                var url = '{{ route("magasinage.destroy", ":magasinage") }}';
                url = url.replace(':magasinage', id);
                $.ajax({
                    type: 'DELETE',
                    url: url,
