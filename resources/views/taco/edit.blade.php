...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Disque  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('vehicle.taco.update',[$vehicle->id,$taco->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-taco :vehicle="$vehicle->id" :taco="$taco">Modifier</x-form-taco>
                                    </form>
...
@section('script')
   ...
   Dropzone.options.docDropzone = {
       ...
       success: function (file, response) {
           ...
           uploadedTacoFileMap[file.name] = response.name
       },
       removedfile: function (file) {
         ...
         if (typeof file.file_name !== 'undefined') {
           ...
           name = uploadedTacoFileMap[file.name]
         }
        ...
        init: function () {
            @if(isset($taco) && $taco->taco_file)
                    var files =
                        {!! json_encode($taco->taco_file) !!}
                        for (var i in files) {
                        var file = files[i]
