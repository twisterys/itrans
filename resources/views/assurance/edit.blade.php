...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Assurance  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('vehicle.assurance.update',[$vehicle->id,$assurance->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-assurance :vehicle="$vehicle->id" :assurance="$assurance">Modifier</x-form-assurance>
                                    </form>
...
@section('script')
   ...
   Dropzone.options.docDropzone = {
       ...
       url: '{{ route('assurance.storeMedia') }}',
       maxFilesize: 15, // MB
       addRemoveLinks: true,
       headers: {
       ...
       removedfile: function (file) {
        ...
        init: function () {
            @if(isset($assurance) && $assurance->assurance_file)
                    var files =
                        {!! json_encode($assurance->assurance_file) !!}
                        for (var i in files) {
                        var file = files[i]
