...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Vehicule  @endslot
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('vehicle.update',['vehicle' => $vehicle->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-vehicle :typeVehicles="$typeVehicle" :vehicle="$vehicle">Modifer</x-form-vehicle>
...
@section('script')
   ...
   Dropzone.options.docDropzone = {
       ...
       removedfile: function (file) {
        ...
        init: function () {
            @if(isset($vehicle) && $vehicle->vehicle_file)
                    var files =
                        {!! json_encode($vehicle->vehicle_file) !!}
                        for (var i in files) {
                        var file = files[i]
