...
@section('title') Visites Technique @endsection
@section('css')
...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Visites Technique  @endslot
         @slot('action')  <a class="btn btn-success " href="{{route('vehicle.technicalVisit.create',$vehicle->id)}}">Nouveau Visite Technique </a>    @endslot
     @endcomponent
...
@section('script')
    ...
    <script type="text/javascript">
        $(function () {
          ...
          var table = $('#datatable-buttons').DataTable({
              processing: true,
              serverSide: true,
              ajax: "{{ route('vehicle.technicalVisit.index',$vehicle->id) }}",
              columns: [
                  {data: 'id', name: 'id'},
                  {data: 'ref', name: 'ref'},
    ...
    table.column(index).search(value, strict).draw()
        ...
        function deleteTechnicalVisit(vehicle,id){
            //alert(id);
            swal.fire({
            title: "Supprimé?",
            icon: 'question',
            text: "Cette suppression engendrera la suppression de cette visite technique!",
            type: "warning",
            showCancelButton: !0,
            ...
            if (e.value === true) {
                ...
                var url = '{{ route("vehicle.technicalVisit.destroy", [":vehicle",":technicalVisit"]) }}';
                url = url.replace(':vehicle', vehicle);
                url = url.replace(':technicalVisit', id);
                $.ajax({
                    type: 'DELETE',
                    url: url,
