...
@section('title') Visite Technique @endsection
@section('css')
...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Visite Technique  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('vehicle.technicalVisit.update',[$vehicle->id,$technicalVisit->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-technical-visit :vehicle="$vehicle->id" :technicalVisit="$technicalVisit">Modifier</x-form-technical-visit>
                                    </form>
...
@section('script')
    ...
    <script>
        var uploadedtechnicalVisitFileMap = {}
   Dropzone.options.docDropzone = {
       ...
       url: '{{ route('technicalVisit.storeMedia') }}',
       maxFilesize: 15, // MB
       addRemoveLinks: true,
       headers: {
       ...
       success: function (file, response) {
           ...
           uploadedtechnicalVisitFileMap[file.name] = response.name
       },
       removedfile: function (file) {
         ...
         if (typeof file.file_name !== 'undefined') {
           ...
           name = uploadedtechnicalVisitFileMap[file.name]
         }
        ...
        init: function () {
            @if(isset($technicalVisit) && $technicalVisit->TechnicalVisit_file)
                    var files =
                        {!! json_encode($technicalVisit->TechnicalVisit_file) !!}
                        for (var i in files) {
                        var file = files[i]
