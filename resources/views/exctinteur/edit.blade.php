...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Extincteur  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('vehicle.exctinteur.update',[$vehicle->id,$exctinteur->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-exctinteur :vehicle="$vehicle->id" :exctinteur="$exctinteur">Modifier</x-form-exctinteur>
                                    </form>
...
@section('script')
   ...
   Dropzone.options.docDropzone = {
       ...
       url: '{{ route('exctinteur.storeMedia') }}',
       maxFilesize: 15, // MB
       addRemoveLinks: true,
       headers: {
       ...
       success: function (file, response) {
           ...
           uploadedExctinteurFileMap[file.name] = response.name
       },
       removedfile: function (file) {
         ...
         if (typeof file.file_name !== 'undefined') {
           ...
           name = uploadedExctinteurFileMap[file.name]
         }
        ...
        init: function () {
            @if(isset($exctinteur) && $exctinteur->exctinteur_file)
                    var files =
                        {!! json_encode($exctinteur->exctinteur_file) !!}
                        for (var i in files) {
                        var file = files[i]
