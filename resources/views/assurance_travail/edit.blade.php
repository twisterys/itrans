...
@section('title') Assurances Travail @endsection
@section('css')
...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Assurances Travail  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('assuranceTravail.update',$assuranceTravail->id)}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-assurance-travail  :assurance="$assuranceTravail">Modifier</x-form-assurance-travail>
                                    </form>
...
@section('script')
   ...
   Dropzone.options.docDropzone = {
       ...
       url: '{{ route('assuranceTravail.storeMedia') }}',
       maxFilesize: 15, // MB
       addRemoveLinks: true,
       headers: {
       ...
       success: function (file, response) {
           ...
           uploadedImportFileMap[file.name] = response.name
       },
       removedfile: function (file) {
         ...
         if (typeof file.file_name !== 'undefined') {
           ...
           name = uploadedImportFileMap[file.name]
         }
        ...
        init: function () {
            @if(isset($assuranceTravail) && $assuranceTravail->assurance_travail_file)
                    var files =
                        {!! json_encode($assuranceTravail->assurance_travail_file) !!}
                        for (var i in files) {
                        var file = files[i]
