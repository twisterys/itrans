...
@section('title') Visite Technique De Disque @endsection
@section('css')
...
@section('content')
    ...
    @component('common-components.breadcrumb')
         @slot('title') Modifier Visite Technique Pour Disque  @endslot
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{route('taco.visiteTechnique.update',[$taco->id,$visiteTechnique->id])}}">
                                        @csrf
                                        @method('PUT')
                                        <x-form-visite-tech-taco :taco="$taco->id" :visiteTechnique="$visiteTechnique">Ajouter</x-form-visite-tech-taco>
                                    </form>
...
@section('script')
    ...
    <script>
        var uploadedVisiTechTacoFileMap = {}
   Dropzone.options.docDropzone = {
       ...
       url: '{{ route('visitTechTaco.storeMedia') }}',
       maxFilesize: 15, // MB
       addRemoveLinks: true,
       headers: {
       ...
       success: function (file, response) {
           ...
           uploadedVisiTechTacoFileMap[file.name] = response.name
       },
       removedfile: function (file) {
         ...
         if (typeof file.file_name !== 'undefined') {
           ...
           name = uploadedVisiTechTacoFileMap[file.name]
         }
        ...
        init: function () {
            @if(isset($visiteTechnique) && $visiteTechnique->VisiteTechniqueTaco_file)
                    var files =
                        {!! json_encode($visiteTechnique->VisiteTechniqueTaco_file) !!}
                        for (var i in files) {
                        var file = files[i]
